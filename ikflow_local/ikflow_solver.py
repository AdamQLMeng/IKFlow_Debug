...
from ikflow_local.config import DEFAULT_TORCH_DTYPE
from ikflow_local.model import IkflowModelParameters, glow_cNF_model
from ikflow_local.evaluation_utils import evaluate_solutions, SOLUTION_EVALUATION_RESULT_TYPE
...
class IKFlowSolver:
    ...
    def load_state_dict(self, state_dict_filename: str):
        ...
        ext = os.path.splitext(state_dict_filename)[-1]
        if ext == '.pkl':
            ...
            checkpoint = torch.load(state_dict_filename, map_location='cpu')
            self.nn_model.load_state_dict(checkpoint['model'])
        else:
            print(f"Incorrect ext of state dict file ({state_dict_filename})!")
