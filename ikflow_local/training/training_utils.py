...
def get_checkpoint_dir(robot_name: str) -> str:
    ckpt_dir_name = f"{robot_name}(checkpoint)--{_datetime_str()}/"
    dir_filepath = os.path.join(config.TRAINING_LOGS_DIR, ckpt_dir_name)
    if not os.path.exists(config.TRAINING_LOGS_DIR):
        os.mkdir(config.TRAINING_LOGS_DIR)
    if not os.path.exists(dir_filepath):
        os.mkdir(dir_filepath)
    return dir_filepath


def get_logs_dir(robot_name: str):
    logs_dir_name = f"{robot_name}(logs)--{_datetime_str()}/"
    dir_filepath = os.path.join(config.TRAINING_LOGS_DIR, logs_dir_name)
    if not os.path.exists(config.TRAINING_LOGS_DIR):
        os.mkdir(config.TRAINING_LOGS_DIR)
    if not os.path.exists(dir_filepath):
        os.mkdir(dir_filepath)
