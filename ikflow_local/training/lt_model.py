...
class IkfLitModel(LightningModule):
    ...
    def configure_optimizers(self):
        ...
        self.lr_scheduler = torch.optim.lr_scheduler.StepLR(
            self.optimizer, step_size=self.hparams.step_lr_every, gamma=self.hparams.gamma, verbose=False
        )
    ...
    def ml_loss_fn(self, batch):
        ...
        output, jac = self.nn_model.forward(x, c=conditional)
    ...
    def validation_step(self, batch, batch_idx, epoch):
        ...
        l2_err = torch.tensor(l2_errors_clamped, dtype=torch.float32).mean().item()
        ang_err = torch.tensor(ang_errors_clamped, dtype=torch.float32).mean().item()
        if batch_idx % self.log_every == 0:
            log_data = {
                "evaluate epoch": epoch,
                "step": batch_idx,
                "l2_errors_clamped": l2_err*1000,
                "ang_errors_clamped": np.rad2deg(ang_err),
            }
            return log_data
        return None
    ...
    def on_validation_epoch_end(self):
        ...
        """
        result = torch.tensor(self.validation_step_outputs).mean(dim=0)
        log_data = {
            "l2_errors_clamped": result[0],
            "ang_errors_clamped": result[1],
        }
        print(log_data)
    ...
    def resume(self, ckpt_file: str):
        start_epoch = 0
        ext = os.path.splitext(ckpt_file)[-1]
        if ext == '.pkl':
            self.ik_solver.load_state_dict(ckpt_file)
        elif ext == '.pth':
            checkpoint = torch.load(ckpt_file, map_location='cpu')
            self.ik_solver.nn_model.load_state_dict(checkpoint['model'])
            self.optimizer.load_state_dict(checkpoint['optimizer'])
            self.lr_scheduler.load_state_dict(checkpoint['scheduler'])
            start_epoch = checkpoint['epoch'] + 1
        print('Resumed from: ' + ckpt_file)
        return start_epoch
